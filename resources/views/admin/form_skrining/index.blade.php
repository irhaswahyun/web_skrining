                                        ...
                                        <tbody>
                                            @foreach ($formSkrinings as $i => $skrining)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button class="btn btn-sm btn-info btn-detail" data-id="{{ $skrining->id }}">Detail</button>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.btn-edit').on('click', function () {
                ...
                var namaSkrining = $(this).data('nama');
                var penyakitId = $(this).data('penyakit-id');
                var editUrl = "{{ route('form_skrining.update', ':id') }}".replace(':id', skriningId);

                $('#edit_nama_skrining').val(namaSkrining);
                $('#edit_id_daftar_penyakit').val(penyakitId).trigger('change');
                $('#editForm').attr('action', editUrl);

                $.get("{{ route('form_skrining.edit', ':id') }}".replace(':id', skriningId), function (data) {
                    $('#edit_pertanyaan_list').empty();
                    $.each(data.pertanyaanList, function (key, value) { // Menggunakan data.pertanyaanList
                        $('#edit_pertanyaan_list').append(`
                            <div>
                                <input type="checkbox" name="pertanyaan_ids[]" value="${value.id}" class="form-check-input" ${value.checked ? 'checked' : ''}>
                                <label class="form-check-label">${value.pertanyaan}</label>
                            </div>
                        `);
                    });
            ...
            $('.btn-detail').on('click', function () {
                ...
                $.get("{{ route('form_skrining.edit', ':id') }}".replace(':id', skriningId), function (data) { // Menggunakan route edit untuk detail karena tidak ada route show
                    $('#detail_nama_skrining').text(data.formSkrining.nama_skrining); // Menggunakan data.formSkrining
                    $('#detail_nama_penyakit').text(data.formSkrining.penyakit ? data.formSkrining.penyakit.Nama_Penyakit : '-'); // Menggunakan data.formSkrining
                    $('#detail_daftar_pertanyaan').empty();
                    if (data.formSkrining.pertanyaan && data.formSkrining.pertanyaan.length > 0) { // Menggunakan data.formSkrining
                        $.each(data.formSkrining.pertanyaan, function (key, value) { // Menggunakan data.formSkrining
            ...
            $('.btn-delete').on('click', function () {
                ...
                var deleteUrl = "{{ route('form_skrining.delete', ':id') }}".replace(':id', skriningId); // Menggunakan route delete
